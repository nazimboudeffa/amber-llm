...
This will download the whole dataset into the `redpajama_v1` folder using `wget` on Unix-like systems or `Invoke-WebRequest` on Windows. We don't need the "common_crawl" subset of the dataset since we replace it with the RefinedWeb dataset.
